...
async function loadNormalRequests(onRecovery) {
  ...
  // กรณีข้อมูลเสียหาย / schemaVersion ไม่ตรง / ID ซ้ำ
  if (stored.status === 'invalid') {
    clearStoredRequests(); // ลบข้อมูลที่เสียหายทิ้ง

    // ต้องเรียก onRecovery เพื่อส่งข้อความแจ้ง UI
    if (typeof onRecovery === 'function') {
      onRecovery('พบข้อมูลในระบบเสียหาย ระบบได้ทำการรีเซ็ตข้อมูลใหม่ให้แล้ว');
    }

    // โหลด seed data ใหม่และบันทึกลง storage
    const freshData = await fetchSeedRequests();
    writeStoredRequests(freshData);
    return freshData;
  }
